...
export default function AuthGate({ children }) {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      if (u) {
        setPhase("syncing");
        const result = await initialSync(u.uid);
        if (result === "reload") {
...
const S = {
  ...
  card: {
    width: "100%", maxWidth: 360, background: "#141A24", border: "1px solid #222B38",
    borderRadius: 18, padding: "32px 26px", boxShadow: "0 20px 60px rgba(0,0,0,0.4)",
    fontFamily: "system-ui, sans-serif",
  },
  brand: {
    fontSize: 24, fontWeight: 800, color: "#34E29B", letterSpacing: "0.18em",
    textAlign: "center", marginBottom: 6,
  },
  subtitle: { fontSize: 13, color: "#8A94A6", textAlign: "center", marginBottom: 24 },
  input: {
    width: "100%", boxSizing: "border-box", background: "#0B0F17", border: "1px solid #222B38",
    borderRadius: 10, padding: "13px 14px", color: "#EAF0F6", fontSize: 15, marginBottom: 10, outline: "none",
  },
  button: {
    width: "100%", background: "#0AA063", border: "none", borderRadius: 10, padding: "14px 0",
    color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer", marginTop: 6,
  },
  error: {
    background: "#3A1620", color: "#FF9AA8", fontSize: 12, borderRadius: 8,
  ...
  spinner: {
    width: 28, height: 28, borderRadius: "50%", border: "3px solid #222B38",
